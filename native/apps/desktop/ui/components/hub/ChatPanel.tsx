...
export function ChatPanel({ chat, channel, onClose = null }: ChatPanelProps) {
    const hub = useApp().hub;
    const { tree } = useStore(hub.store);
    const { messages, loading, loadingOlder, failed, replyTo, newFrom, images, sending } = useStore(chat.store);
    ...
    const label = voice ? channel.name : `#${channel.name}`;
    ...
    const attach = (files: File[]) => {
        if (canSend && files.length > 0) {
            void chat.attach(files);
        }
    };

    const onPaste = (event: ClipboardEvent<HTMLTextAreaElement>) => {
        const data = event.clipboardData;
        const pasted = data.files.length > 0
            ? [...data.files]
            : [...data.items].map(item => item.getAsFile()).filter((file): file is File => file !== null);
        const pictures = pasted.filter(file => file.type.startsWith('image/'));
    ...
    const send = async () => {
        ...
        if ((body.trim() === '' && images.length === 0) || sending) {
            return;
        }
    ...
    return (
        <section
            className={`glass relative flex min-h-0 flex-1 animate-fade-in flex-col gap-3 p-4 ${dropping ? 'border-brand/60' : ''}`}
            onDragOver={onDragOver}
            onDragLeave={event => event.currentTarget.contains(event.relatedTarget as Node | null) || setDropping(false)}
            onDrop={onDrop}
        >
            <div className="flex items-center gap-2 border-b border-line pb-3">
                {voice ? <span className="text-online"><Icon name="speaker" size={14} /></span> : <span className="font-mono text-[14px] text-lilac-2">#</span>}
            ...
            <div ref={list} className="scroll-thin flex min-h-0 flex-1 flex-col gap-2.5 overflow-y-auto pr-1" onScroll={onScroll} onLoadCapture={stickToEnd}>
                ...
                {! loading && messages.length === 0 && failed && (
                    <div className="m-auto flex flex-col items-center gap-2 text-center">
                        <p className="text-[13px] text-danger">Não deu para carregar as mensagens de {label}.</p>
                        <button className="btn-ghost" type="button" onClick={() => void hub.attempt(() => chat.open(channel))}>Tentar de novo</button>
                    </div>
                )}

                {! loading && messages.length === 0 && ! failed && (
                    <p className="m-auto text-center text-[13px] text-ink-dim">Nenhuma mensagem ainda em {label}.</p>
                )}

                {messages.map(message => (
                    <MessageRow key={message.id} chat={chat} message={message} mine={chat.isMine(message)} canDelete={chat.canDelete(message)} unreadMark={message.id === newFrom} />
                ))}
            </div>
            ...
            {replyTo && (
                <div className="flex items-center gap-2 rounded-[10px] border border-line-strong bg-row px-3 py-1.5 text-[12px]">
                    ...
                    <span className="min-w-0 flex-1 truncate text-ink-dim">{replyTo.body || 'imagem'}</span>
            ...
            {images.length > 0 && (
                <div className="flex flex-wrap gap-2">
                    {images.map(image => (
                        <span key={image.id} className="relative flex-none">
                            <img className="size-16 rounded-[10px] border border-line-strong object-cover" src={image.preview} alt={image.file.name} />
                            <button className="absolute -top-1.5 -right-1.5 flex size-5 cursor-pointer items-center justify-center rounded-full border border-line-strong bg-[rgba(16,13,26,0.96)] text-ink-icon hover:text-danger" type="button" title="Tirar esta imagem" disabled={sending} onClick={() => chat.detach(image.id)}>
            ...
            <form className="flex items-end gap-2" onSubmit={event => { event.preventDefault(); void send(); }}>
                ...
                />
                <button className="btn-icon flex-none" type="button" title="Anexar imagem (ou cole com Ctrl+V, ou arraste para cá)" disabled={! canSend || images.length >= Chat.MAX_IMAGES} onClick={() => picker.current?.click()}>
                ...
                <textarea
                    ...
                    value={draft}
                    disabled={! canSend}
                    placeholder={! canSend ? 'Você não pode enviar mensagens neste canal' : replyTo ? `Respondendo ${replyTo.user.name}` : `Escreva em ${label}`}
                ...
                />
                <button className="btn-primary px-4 py-2.5 text-[13px]" type="submit" disabled={! canSend || sending || (draft.trim() === '' && images.length === 0)}>{sending ? 'Enviando…' : 'Enviar'}</button>
            </form>
        </section>
    );
}
