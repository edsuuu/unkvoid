...
python3 check-language.py
"""
import pathlib
import re
import sys

# native/apps/desktop/check-language.py -> quatro níveis até a raiz do repositório.
...
FONTES = [
    ...
    ('native/apps/desktop/ui', '*.js', '//'),
    ('sfu/src', '*.ts', '//'),
]

DECLARACAO = re.compile(
    r'\b(?:let|const|var|static|fn|struct|enum|function|class|type|mut|readonly)\s+'
    r'(?:mut\s+)?([A-Za-z_$][\w$]*)'
)
