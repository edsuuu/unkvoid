# Para o agente no Mac — build 0.0.21 do macOS
...
subiu para 0.0.21 e lê o atualizador em `https://unkvoid.com/downloads/latest.json`).

O Linux 0.0.21 já está publicado. Falta o macOS, e ele só sai de um Mac.
...
- `dmg/Unkvoid_0.0.21_aarch64.dmg` — o instalador para quem baixa pela primeira vez.
...
./publish-release.sh darwin-aarch64-dmg ../../target/release/bundle/dmg/Unkvoid_0.0.21_aarch64.dmg
```
...
2. Crie uma sala, copie o código, entre com outro aparelho (o Linux 0.0.21 serve) e
...
- Não mexa em `tauri.conf.json` além do que já está na branch. A versão 0.0.21 é a
...
solução: o build 0.0.21 acima é que resolve.
